...
export const CommandErrorHandler: GuardFunction<CommandInteraction> = async (interaction, client, next) => {
  ...
  try {
    ...
    logger.error(`There was an error executing ${interaction.commandName} in ${interaction.guild?.name}. ${err}`);
  }
};
